export const getEmailHTML = (htmlTemplate: string, code: string) => {
    ...
    const warning = `
      <div style="border-top: 1px solid rgba(0,0,0, 0.1);  padding: 1rem 2rem;">
        <p style="font-size: 14px; color:rgba(0,0,0, 0.6);">Itaaj Realty nunca enviará un correo electrónico solicitando que revele o verifique su contraseña, tarjeta de crédito o número de cuenta bancaria.</p>
      </div>
    `;
  
    const footer = `
      <p style="max-width: 500px; margin: 0 auto; text-align: center; font-family: sans-serif; font-size: 14px; color: rgba(0,0,0, 0.6);">Itaaj Realty, Inc,. &copy; 2024, Itaaj Realty. Todos los derechos reservados. Itaaj Realrt es una marca comercial registrada</p>
    `;
...
${header}\n${htmlTemplate}\n${warning}\n${footer}</div>`.replace(
  ...
  export const EmailSubjects = {
    verification: 'Verificación de correo Itaaj Realty',
  ...
  export const EmailTemplates = {
    verification: `
      <div style=" padding: 1rem 2rem;">
        <h3 style="font-size: 1.2rem; color: rgba(0,0,0, 0.8);">Inicia sesión en Itaaj Realty</h3>
        <p style="font-size: 14px;">Ingresa el código de acceso en la plataforma para iniciar sesión de forma segura. Este codigo expirará en 15 minutos.</p>
        <h2>{code}</h2>
      </div>
    `,
