...
export const Access = async () => {
  ...
  try {  
    ...
    const response = await axios.post('https://itaaj.nevobit.io/api/v1/auth/access', {}, headers);
...
export const Login = async (body: any) => {
  try {
    const response = await axios.post('https://itaaj.nevobit.io/api/v1/auth/login', body);
...
export const Register = async (body: any) => {
  try {
    const response = await axios.post("https://itaaj.nevobit.io/api/v1/auth/register", body);
    const { token, user } = response.data;
    localStorage.setItem("tokene", token);
    Cookies.set('tokene', token, { expires: 7 });
