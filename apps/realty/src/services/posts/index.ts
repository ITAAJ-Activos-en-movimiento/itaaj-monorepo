export const getPostsApi = async () => {
  const response = await fetch("https://itaaj-api-gt8a.onrender.com/api/v1/posts", {
    cache: "no-store",
  });
...
export const getPostBySlugApi = async (slug: string) => {
  const response = await fetch(`https://itaaj-api-gt8a.onrender.com/api/v1/posts/${slug}`, {
    cache: "no-store",
  });
