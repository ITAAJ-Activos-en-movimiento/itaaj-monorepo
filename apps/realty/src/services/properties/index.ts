export const properties = async ({ page, limit }: { page: number, limit: number }) => {
  try {
    const response = await fetch(
      `https://itaaj.nevobit.io/api/v1/properties?page=${page}&limit=${limit}`,
      { cache: "no-store" }
    );
...
export const propertiesByDevelopment = async (development: string) => {
  try {
    const response = await fetch(
      `https://itaaj.nevobit.io/api/v1/properties/${development}`,
      { cache: "no-store" }
    );
...
export const propertiesBySlug = async (slug: string) => {
  try {
    const response = await fetch(
      `https://itaaj.nevobit.io/api/v1/property/${slug}`,
      { cache: "no-store" }
    );
