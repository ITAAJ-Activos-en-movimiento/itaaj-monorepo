...
const Cform = ({ slug, closeModal, prevmsg, children }: Props) => {
  ...
  const sendData = async (e: any) => {
    ...
    try {
      if (info.email != "") {
        await axios.post("https://itaaj-realty.onrender.com/api/v1/messages", {
          ...info,
        });
  ...
  return (
    <form className={styles.form} onSubmit={sendData}>
      ...
      <input
        ...
        value={info.email}
        id="email"
        placeholder="Tu e-mail(obligatorio)"
        onChange={(e) =>
      ...
      <label htmlFor="terms">
        ...
        <p>
          Acepto las condiciones de uso, la información basica de Proteccion de
          Datos y darme de alta en itaaj
        </p>
      </label>
      <button
        disabled={
          info.name.length < 3 || info.phone.length < 4 || info.email.length < 5
        }
