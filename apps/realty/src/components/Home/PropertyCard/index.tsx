...
const ProeprtyCard = ({
  ...
  const info =
    loc == "developments"
      ? `/developments/${slug}`
      : `/${
          alsoRent == true ? "rentar" : "comprar"
        }/viviendas/mexico/${slug}/d`;
  return (
    <Link href={info}>
      ...
      <p>
        <strong>{changeLanguage(type)}</strong> {address} en{" "}
        <strong>{city}</strong>
      </p>
      <span className={styles.info}>
        <span>{bedrooms} habs</span> &middot; <span>{bathrooms} baños</span>{" "}
        &middot;
      </span>
      <ul className={styles.options}>
        <li>
          {" "}
          <Mail size={20} color="var(--second-color)" />{" "}
        </li>
        <li className={styles.phone}>
          {" "}
          <Phone size={20} color="var(--second-color)" />{" "}
        </li>
        <li>
          {" "}
          <Heart size={20} color="#e84079" />{" "}
        </li>
      </ul>
    </Link>
  );
