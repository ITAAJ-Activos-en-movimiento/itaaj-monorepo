    ...
    useEffect(() => {
        ...
        if (searchParams && searchParams.address) {
            ...
            const calculatePolygon = async (params: any) => {
                const { address, maxTime, transport } = params;
                try {
                  const response = await fetch('https://itaajrealty.com/api/api/v1/search/calculate-polygon', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ address, maxTime, transport }),
                  });
                
                  const { data: dataProps } = await axios("https://itaajrealty.com/api/api/v1/properties?page=1&limit=40");
                  const newLocations = dataProps.items.map((property: any) =>  ({ name: property.name, lat: property.location.latitude, lng: property.location.longitude}));
