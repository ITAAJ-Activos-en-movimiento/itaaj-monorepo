...
const RegisterModalPhone = () => {
  ...
  const handleGoogleSuccessPhone = async (
    credentialsResponse: CredentialResponse
  ) => {
    if (credentialsResponse.credential) {
      const token_id = credentialsResponse.credential;
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.post(
        "https://itaaj-api-v0.onrender.com/api/v1/proposals",
        proposal
      );
      // console.log(data);
      setLoading(false);
  ...
  useEffect(() => {
    setProposal((prev) => ({
      ...prev,
      ["enganche"]: Number(prev.economic_proposal * 0.2),
    }));
    setProposal((prev) => ({
      ...prev,
      ["rest"]: Number(prev.economic_proposal) - Number(prev.enganche),
    }));
    ...
    <GoogleOAuthProvider clientId="259968467063-s0d076kvsf87se4bgmofbll4ivf6gom3.apps.googleusercontent.com">
      ...
      <div
        className={
          register == null || register == "close"
            ? styles.modal
            : `${styles.modal} ${styles.open_modal}`
        }
      >
        ...
        <form onSubmit={onSubmit} className={styles.form}>
          ...
          <button className={styles.proposal_btn}>
