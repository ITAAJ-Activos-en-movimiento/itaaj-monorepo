        ...
        <option value={col.value}>{col.label}</option>
                ...
                <div className={styles.results}>
                    ...
                    <h3>Propiedades Analizadas: {data?.amount}</h3>
                        ...
                        <h3>Precio por Metro Cuadrado: {DivisaFormater({ value: data?.precioPorMetro})}</h3>
                        <h3>Desviación Estandar: {DivisaFormater({ value: data?.desviacionEstandar})}</h3>
                        <h3>Precio Promedio: {DivisaFormater({ value: data?.precioPromedio})}</h3>
            ...
            <Table.Body<{ encabezado: string; precio: number, url: string }> data={data?.properties.map((property: { encabezado: string, urlCorrectaPropiedad: string, precios: { vista: { precio: number } } }) => ({encabezado: property.encabezado, url: property.urlCorrectaPropiedad, precio: property.precios.vista.precio}))} render={(property, index) => <PropertyRow properties={property} index={index} key={property.encabezado}  />}/>
